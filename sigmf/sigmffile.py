...
class SigMFCollection(SigMFMetafile):
    ...
    def __init__(self, metafiles=None, metadata=None, path=None, skip_checksums=False):
        ...
        path -- path of the datafiles of the collection
        """
    ...
    def verify_stream_hashes(self):
        ...
        for stream in streams:
            ...
            if path.isfile(f"{self.path}/{metafile_name}"):
                new_hash = sigmf_hash.calculate_sha512(filename=f"{self.path}/{metafile_name}")
                if old_hash != new_hash:
                    raise SigMFFileError(
                        f"Calculated file hash for {self.path}/{metafile_name} does not match collection metadata."
                    )
    ...
    def set_streams(self, metafiles):
        ...
        for metafile in self.metafiles:
            if metafile.endswith(".sigmf-meta") and path.isfile(f"{self.path}/{metafile}"):
                stream = {
                    "name": get_sigmf_filenames(metafile)["base_fn"],
                    "hash": sigmf_hash.calculate_sha512(filename=f"{self.path}/{metafile}"),
                }
                streams.append(stream)
            else:
                raise SigMFFileError(f"Specifed stream file {self.path}/{metafile} is not a valid SigMF Metadata file")
    ...
    def get_SigMFFile(self, stream_name=None, stream_index=None):
        ...
        if metafile is not None:
            return fromfile(f"{self.path}/{metafile}", skip_checksum=self.skip_checksums)
...
def fromfile(filename, skip_checksum=False):
    ...
    if (ext.lower().endswith(SIGMF_COLLECTION_EXT) or not path.isfile(meta_fn)) and path.isfile(collection_fn):
        ...
        return SigMFCollection(metadata=metadata, path=dir_path, skip_checksums=skip_checksum)
